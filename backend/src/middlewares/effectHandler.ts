...
import { isAppError, isEffect } from "#middlewares/utils/typeGuards";

/**
 * Type for handlers that can return:
 * 1. Effect results (old style, for backward compatibility)
...
) => Promise<Effect<unknown> | undefined | unknown>;
 ...
 *   const effect = getUser(userId);
 *   const result = await runEffect(effect);
 *   return result; // Returns Effect<User>
 * }
 ...
 */
export function handleEffect(handler: EffectHandler) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // This handles:
      // 1. Old style: Effect<T> (Success/Failure from runEffect)
      // 2. matchResponse: Failure part of U | Failure discriminated union
      if (isEffect(result)) {
        handleEffectResult(res, result, req.method);
        return;
      }
